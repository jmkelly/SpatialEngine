...
public static class ProjectApplier
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        ArgumentNullException.ThrowIfNull(settings);
        var datasets = await gateway.ListDatasetsAsync(settings.Store, null, cancellationToken);
        var maps = await gateway.ListMapsAsync(cancellationToken);
        ...
        await PublishMapsAsync(gateway, project, settings, maps, cancellationToken);
        return plan;
    }

    /// <summary>Reads the host's datasets and maps into a version-1 project.</summary>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return new SpatialProject(
            ...
            maps.Select(ToProjectMap).ToArray());
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var existing = maps.FirstOrDefault(candidate =>
                string.Equals(candidate.Name, map.Name, StringComparison.Ordinal));
            await gateway.PutMapAsync(BuildMap(map, existing), settings.Token, cancellationToken);
        }
    }

    private static Map BuildMap(ProjectMap map, Map? existing)
    {
        var service = ParseKind(map.Kind);
        return new Map(
            map.Name,
            map.Store,
            BuildLayers(map, existing, service),
            [service],
            map.Description,
            map.Copyright);
    }
    ...
    private static List<MapLayer> BuildLayers(ProjectMap map, Map? existing, MapService service)
    {
        ...
        foreach (var layer in map.Layers ?? [])
        {
            ...
            layers.Add(new MapLayer(layer.Dataset, layerId, layer.Name, LowerStyle(layer), kind));
        }

        return layers;
    }

    private static Dictionary<string, int> ExistingLayerIds(Map? map)
    {
    ...
    private static ProjectMap ToProjectMap(Map map) => new(
        map.Name,
        PrimaryKind(map.Services),
        map.Store,
        map.Description,
        map.Copyright,

...
internal static class MapOfflineRejects
{
    ...
    internal static Task<IResult> MapEstimateExportTileSize(
        GeoServicesCatalog catalog, IMapRegistry registry, string service, CancellationToken cancellationToken) =>
        RejectAsync(catalog, registry, service, "MapServer", MapService.Map, "The 'estimateExportTileSize' operation is not supported: there is no offline tile package to size — packaging needs a job model this host does not have (ADR-0033); the root advertises exportTilesAllowed:false. Render tiles live via tile/{z}/{y}/{x}.", cancellationToken);
    ...
    internal static Task<IResult> ImageExportTiles(
        GeoServicesCatalog catalog, IMapRegistry registry, string service, CancellationToken cancellationToken) =>
        RejectAsync(catalog, registry, service, "ImageServer", MapService.Image, "The 'exportTiles' operation is not supported: offline raster tile packaging needs a packaging job model, and this host serves live-rendered imagery only (ADR-0033). Render one image live via exportImage.", cancellationToken);
    ...
    internal static Task<IResult> ImageEstimateExportTileSize(
        GeoServicesCatalog catalog, IMapRegistry registry, string service, CancellationToken cancellationToken) =>
        RejectAsync(catalog, registry, service, "ImageServer", MapService.Image, "The 'estimateExportTileSize' operation is not supported: there is no offline tile package to size — packaging needs a job model this host does not have (ADR-0033). Render one image live via exportImage.", cancellationToken);
    ...
    internal static Task<IResult> GenerateKml(
        GeoServicesCatalog catalog, IMapRegistry registry, string service, CancellationToken cancellationToken) =>
        RejectAsync(catalog, registry, service, "MapServer", MapService.Map, "The 'generateKml' operation is not supported: no KML surface is served.", cancellationToken);

    /// <summary>Rejects the MapServer <c>kml</c> image (S4 kml-image-map-service/).</summary>
    internal static Task<IResult> KmlImage(
        GeoServicesCatalog catalog, IMapRegistry registry, string service, CancellationToken cancellationToken) =>
        RejectAsync(catalog, registry, service, "MapServer", MapService.Map, "The 'kml' image resource is not supported: no KML surface is served.", cancellationToken);

    /// <summary>Rejects the async job surface (<c>jobs</c>, one job, its results and inputs).</summary>
    internal static Task<IResult> Jobs(
        GeoServicesCatalog catalog, IMapRegistry registry, string service, CancellationToken cancellationToken) =>
        RejectAsync(catalog, registry, service, "MapServer", MapService.Map, "The 'jobs' resource is not supported: long-running work runs as cancellable tasks, not observable jobs — there is no job model to poll (ADR-0033). Await the synchronous export or tile response instead.", cancellationToken);

    private static async Task<IResult> RejectAsync(
        GeoServicesCatalog catalog, IMapRegistry registry, string service,
        string serverType, MapService mapService, string message, CancellationToken cancellationToken)
    {
        try
        {

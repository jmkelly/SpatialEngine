...
internal static class AppComposition
{
    ...
    private static void ConfigureHostRemoteAccess<T>(IResourceBuilder<T> host)
    ...
    private static void ConfigureWorkbenchRemoteAccess<T>(IResourceBuilder<T> workbench)
        where T : IResourceWithEndpoints, IResourceWithEnvironment
    {
        ConfigureWorkbenchEndpoint(workbench);
        ConfigureAllowedHosts(workbench);
    }

    private static void ConfigureWorkbenchEndpoint<T>(IResourceBuilder<T> workbench)
        where T : IResourceWithEndpoints, IResourceWithEnvironment
    {
        var devBind = Environment.GetEnvironmentVariable("SPATIAL_DEV_BIND");
        if (!string.IsNullOrWhiteSpace(devBind))
    ...
    private static void ConfigureAllowedHosts<T>(IResourceBuilder<T> workbench)

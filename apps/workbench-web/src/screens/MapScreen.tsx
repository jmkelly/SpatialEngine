...
/**
 * The workbench map (Phase 10): a MapLibre canvas over an empty style (no
 * network tiles — the workbench renders engine data only), with the loaded
 * dataset as point/line/fill layers and the invocation/persisted-result layer
 * in a contrasting colour. Geometry attributes are canonical SGEOM bytes
 * decoded by src/sgeom.ts; the client holds no spatial algorithms.
 */
export function MapScreen() {
  ...
  useEffect(() => {
    ...
    map.on("mousemove", (event) => {
      const found = map.queryRenderedFeatures(event.point, { layers: ["feature-points", "feature-lines", "feature-fills"] });
...
function datasetOptions(catalogue: string[]): string[] {
  ...
  return ["demo.points", "demo.cities"];
}

function dataStyle(): StyleSpecification {
  return {
    version: 8,
    sources: {
      features: { type: "geojson", data: { type: "FeatureCollection", features: [] } as GeoJSON.FeatureCollection },
    ...
    layers: [
      ...
      { id: "feature-points", type: "circle", source: "features", paint: { "circle-color": "#4fc3f7", "circle-radius": 5, "circle-stroke-color": "#0b0f14", "circle-stroke-width": 1 } },
...
/** Sets a GeoJSON source's data when it is ready; fires once the style is ready otherwise. */

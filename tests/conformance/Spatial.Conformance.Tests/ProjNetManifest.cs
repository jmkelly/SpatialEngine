...
/// <summary>
/// The plugin package manifest for the ProjNet transformation worker, derived
/// from the provider's contract descriptors so the package surface can never
/// drift from the contracts it implements (the worker host's activation
/// compatibility check then passes by construction; the Phase 5 fixture tests
/// exercise the divergence gate deliberately).
/// </summary>
internal static class ProjNetManifest
{
    public const string AssemblyName = "Spatial.Transformations.ProjNet.dll";
    public const string ProviderType = "Spatial.Transformations.ProjNet.ProjNetTransformationsProvider";

    public static PluginManifest Build()
    {
        var provider = new ProjNetTransformationsProvider();

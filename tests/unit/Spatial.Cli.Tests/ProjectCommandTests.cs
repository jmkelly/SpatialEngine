...
public sealed class ProjectCommandTests
{
    ...
    public async Task Apply_ingests_missing_reuses_existing_and_publishes_maps()
    {
        ...
        Assert.Equal(PublicationKind.Map, put.Publication.Kind);
        Assert.Equal(2, put.Publication.Layers.Count);
    }
    ...
    public async Task Apply_preserves_an_existing_publications_layer_ids()
    {
        ...
        var gateway = new FakeSpatialGateway
        {
            ...
            [
                new Publication(
                    ...
                    "memory",
                    [
                        new PublicationLayer("public.a", 0),
                        new PublicationLayer("public.b", 7),
                    ]),
            ],
        };
        ...
        Assert.Equal(ExitCodes.Success, run.ExitCode);
        var layers = Assert.Single(gateway.PutCalls).Publication.Layers;
    ...
    public async Task Export_uses_mixed_geometry_when_a_layer_has_no_recognised_style()
    {
        var gateway = new FakeSpatialGateway
        {
            Publications =
            [
                new Publication("World", PublicationKind.Map, "memory", [new PublicationLayer("public.world", 0)]),
            ],
        };
        var path = TempPath();

...
public sealed class CommandTests
{
    ...
    public async Task Map_show_describes_layers_and_endpoint()
    {
        ...
        gateway.PublicationsByName = new Dictionary<string, Publication>(StringComparer.Ordinal)
        {
            ["World"] = new Publication(
                ...
                PublicationKind.Map,
                "memory",
                [new PublicationLayer("public.world", 0, "Countries", MapLibreStyleBuilder.Lower(new DrawRecipe(), GeometryFamily.Polygon))]),
        };
    ...
    public async Task Map_export_url_prints_only_the_endpoint()
    {
        ...
        gateway.PublicationsByName = new Dictionary<string, Publication>(StringComparer.Ordinal)
        {
            ["World"] = new Publication("World", PublicationKind.Feature, "memory", []),
        };

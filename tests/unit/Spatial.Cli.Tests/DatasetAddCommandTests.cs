...
public sealed class DatasetAddCommandTests
{
    ...
    /// <summary>
    /// A gateway that returns an ingest outcome carrying a publication so the
    ...
    private sealed class PublishingGateway(FakeSpatialGateway inner) : ISpatialGateway
    {
        ...
        public Task<Publication> PutPublicationAsync(Publication publication, string? adminToken, CancellationToken cancellationToken = default) =>
            inner.PutPublicationAsync(publication, adminToken, cancellationToken);

        public Task<bool> DeletePublicationAsync(string name, string? adminToken, CancellationToken cancellationToken = default) =>
            inner.DeletePublicationAsync(name, adminToken, cancellationToken);

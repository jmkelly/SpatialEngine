...
/// <summary>
/// The Esri-docs parity replay suite, slice A: GeometryServer (T-064).
/// Every case in <c>tests/fixtures/esri-docs/geometryserver/</c> carries a
/// doc-verbatim request (method + path + wire-encoded query string) and the
/// doc-verbatim Esri response; the suite replays the stored query string
...
/// Later slices reuse this harness: FeatureServer/query (T-069),
/// MapServer (T-070), ImageServer + edge-cases (T-071).
/// </summary>
/// <remarks>
/// ProjNet-vs-Esri-PE note: <c>project</c> replays agree within the scaled
/// tolerance because both engines evaluate the same closed-form projections
/// inside one CRS family; datum steps are deliberately out of scope (the
/// engine ships no datum tables — see the fixtures README and
/// <c>findTransformations</c>). Where the host honestly differs (input
/// aliases, arc segmentation, ring orientation) the fixture records the
/// delta in <c>knownDeltas</c> instead of chasing parity; behaviour changes
/// are never smuggled in through this suite.
/// </remarks>
public sealed class EsriDocsReplayTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public async Task Docs_request_replays_against_our_host(string name)
    {
        ...
        Compare(fixture.GetProperty("expect"), body.RootElement, "$", deltas);
        Assert.True(
            deltas.Count == 0,
    ...
    private static void Compare(JsonElement expected, JsonElement actual, string path, List<string> deltas)
    {
        ...
        switch (expected.ValueKind)
        {
            case JsonValueKind.Object:
                foreach (var property in expected.EnumerateObject())
                {
                    ...
                    {
                        Compare(property.Value, child, $"{path}.{property.Name}", deltas);
                    }
                }
            ...
            case JsonValueKind.Array:
                ...
                {
                    Compare(wantItems[i], gotItems[i], $"{path}[{i}]", deltas);
                }

...
- `Map`/`MapService`/`MapLayer`/`MapLayerKind` and the ingest records
  are core-typed (ADR-0052); no protocol or provider type crosses.
...
**Map registry specifics (ADR-0052):** `Spatial.Provider.Maps`
...
versioned JSON file written atomically, and reads a pre-ADR-0052
...
host injects `source-layer` when it assembles a render document. An enabled

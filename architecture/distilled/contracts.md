...
| `IngestAsync` (`IDatasetIngest`) | `IngestRequest`, `FeatureBatch` pages | atomic create + load in one transaction; identity mode `None`/`Auto`/`Source`; additive capability (ADR-0041). The host ingest route also accepts `sourceSrid` and reprojects pages through `ICoordinateTransforms` before load (ADR-0047) |
| `ListAsync` / `GetAsync` / `PutAsync` / `DeleteAsync` (`IPublicationRegistry`) | publication name / `Publication` | runtime service registry (ADR-0041): declared entries immutable, runtime entries persisted; `Publication` carries name, kind, store and stable-id layers, each with an optional `LayerStyle` (ADR-0047) |
...
- `Publication`/`PublicationKind`/`PublicationLayer` (with the optional `LayerStyle`, ADR-0047) and the ingest records
...
versioned JSON file written atomically. `Auto`-identity ingest adds a
`GENERATED BY DEFAULT AS IDENTITY` primary key; `Source` uses a named integer
field; `None` is data-only. `FeatureId.Unassigned` on an add means the store
assigns the key (ADR-0043). `PublicationLayer.Style` (ADR-0047) is validated
(hex colour, opacity 0–1) and persisted verbatim.

**GeoServices specifics:** a `PublicationKind.Feature` publication is a
`FeatureServer`; a `PublicationKind.Map` publication is a MapServer M0
(root, `layers`, layer metadata, `query`) carrying the layer's `LayerStyle`
as `drawingInfo` (ADR-0047). MapServer advertises `Query,Data` only —
export/tiles are M2/M3 and not yet served.
